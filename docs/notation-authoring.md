...
press H to hide the hint. The same stage is at `/lawyer/outline`, which switches among the bundled retainer and a sample
motion (`?doc=retainer`, `motion`). This command does not lint the outline and does not change PDF numbering.
