...
///
/// The canonical seed runs on every boot, so two `web` processes can start
/// together — and `person.role` is the authorization root, so a mailbox
/// that forks into two rows is one human carrying two roles.
///
