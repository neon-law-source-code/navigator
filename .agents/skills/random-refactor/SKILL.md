...
- **Clear, small, evidence-backed findings:** smallest present-tense change, covering test first, one file. Run
  `cargo fmt`, the focused test, and `cargo run -p cli --quiet -- project gate`. Use `rtk cargo test` and `rtk cargo clippy`
  for compact focused output when available; RTK does not speed compilation. For Rust behavior changes, also clippy with warnings denied and crate tests.
