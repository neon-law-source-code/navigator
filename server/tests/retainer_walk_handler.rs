...
async fn step_get_prefill_is_scoped_to_current_notation() {
    ...
    store::answers::record(
        ...
        .in_notation(other_notation.id, "person__client")
...
async fn start_post_rejects_missing_at_in_client_email_with_validation_error() {
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/x-www-form-urlencoded")
                .body(Body::from(
                    "client_email=not-an-email&retainer_template_code=onboarding__estate",
                ))
                .unwrap(),
        )
        .await
    ...
    assert!(
        loc.contains("retainer_template_code=onboarding__estate"),
        "{loc}"
    );
    ...
    assert!(html.contains("onboarding__estate"), "{html}");
}
