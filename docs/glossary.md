...
narration units, and `/app/outline` shows the bundled onboarding and offboarding letters on that stage (`?doc=`). A
notation the firm has given a client is the same stage at `/app/projects/{code}/{notation_id}/outline`. `N123` holds
every kind that must carry an outline to its depth-1 scheme.
