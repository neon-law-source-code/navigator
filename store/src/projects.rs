...
/// assets filed on these projects). A matter clears a side either by a
/// notation bound to a template whose declared `kind` classifies that way, or
/// by an `assets` row directly carrying such a kind — a lawyer uploading and
/// classifying a signed letter counts exactly as much as one generated by a
/// questionnaire walk. Both keys go through [`template_opens_a_matter`] and
...
///
/// `notations`, `templates`, and `assets` are all Surreal-resident since
/// ENG-121, but this stays three batched queries either way — one round trip
/// per table, not one round trip per Project.
///
