...
async fn only_neon_advertises_the_delete_your_data_gateway() {
    ...
    let advertised = advertised_paths(&document(&app).await);
    assert!(
        advertised.iter().any(|path| path == "/delete-your-data"),
        "Neon's own llms.txt must advertise its gateway: {advertised:?}"
    );
...
async fn only_neon_advertises_the_immigration_gateway() {
    ...
    let advertised = advertised_paths(&document(&app).await);
    assert!(
        advertised.iter().any(|path| path == "/immigration"),
        "Neon's own llms.txt must advertise its gateway: {advertised:?}"
    );
...
async fn only_neon_advertises_the_estate_planning_gateway() {
    ...
    let advertised = advertised_paths(&document(&app).await);
    assert!(
        advertised.iter().any(|path| path == "/estate-planning"),
        "Neon's own llms.txt must advertise its gateway: {advertised:?}"
    );
...
async fn only_neon_advertises_the_accidents_gateway() {
    ...
    let advertised = advertised_paths(&document(&app).await);
    assert!(
        advertised.iter().any(|path| path == "/accidents"),
        "Neon's own llms.txt must advertise its gateway: {advertised:?}"
    );
