...
rather than assembled from answers. A body with a signature affordance (a placeholder, a signature heading, or a
structured manual signing line) must declare both machines regardless of `kind`: a binding instrument without a
questionnaire and workflow is half a document, and a half-built document should never reach a client. If either kind
declares one machine, it must declare the other. This is a guardrail, not a nicety.
