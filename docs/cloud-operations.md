...
Top-level files in `docs/` are already published at `/documents/:slug` by `portal::docs`. The site bakes the docs into
the binary with `include_str!`, renders markdown under the firm brand, and rewrites top-level doc links to site routes.
That gives every maintainer and LLM the same documentation surface.
