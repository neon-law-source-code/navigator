//! `/estate-planning` — Neon's own gateway page to Vesta Estate Planning.
//!
...
//! `/immigration`, `/business`, `/services`, and `/disputes` off a
//! house-brand host. See `neon::firm_copy::estate_planning_gateway` for the
//! content and `views::brand::BrandKey::publishes_firm_path` for the gate.
...
/// `GET /estate-planning` on a Neon host returns 200, wears Neon chrome, and
/// carries the approved original copy — the destination named factually,
/// the exact $5,000-once/unlimited-edits pricing, and a destination-named
/// CTA whose href resolves to this deployment's own Vesta host.
#[tokio::test]
async fn the_gateway_renders_on_neon_hosts_with_neon_chrome_and_the_approved_copy() {
    ...
    for host in [None, Some("www.neonlaw.com")] {
        let (status, body) = get_on_host(&app, "/estate-planning", host).await;
        assert_eq!(status, StatusCode::OK, "{host:?}: {body}");
        for expected in [
            "<title>Neon Law | Estate Planning</title>",
            r#"content="Estate planning | Neon Law" property="og:title""#,
            "A plan you can keep current.",
            "Vesta Estate Planning",
            "Shook Law PLLC",
            "$5,000 once",
            "unlimited edits for life",
        ...
        assert!(
            body.contains(r#"href="https://www.vestaestateplanning.com""#),
            "{host:?}: the CTA does not open the production Vesta destination: {body}"
        );
    }
}
...
async fn a_staging_deployment_resolves_the_cta_to_the_staging_sibling() {
    ...
    let (status, body) = get_on_host(&app, "/estate-planning", Some("www.neonlaw.com")).await;
    assert_eq!(status, StatusCode::OK, "{body}");
    assert!(
        body.contains(r#"href="https://staging.vestaestateplanning.com""#),
...
/// including Vesta's own — because only Neon's `publishes_firm_path` admits
/// it.
#[tokio::test]
async fn the_gateway_is_404_on_every_non_neon_brand_host() {
    ...
    for host in [
        ...
        let (status, body) = get_on_host(&app, "/estate-planning", Some(host)).await;
        assert_eq!(
            status,
