...
same `/app/admin` route bypass as the people directory; Lawyer and Clerk stay denied. Owner edits a system-wide or
...
Owner. `/app/owner` is not an Admin surface. `/app/admin/brands` is: Owner and Admin reach the registry, and an Admin
DRI may change that Firm's typeface and palette from the closed catalogs, and replace its logo and uploaded font; a
system-wide brand stays Owner's.
