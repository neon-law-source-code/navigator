...
reliance, not where the software runs. Each version
converts to `AGPL-3.0-only` four years after it is published, and § 13 then obliges an operator who modified
that version to offer those users — its own remote users — the corresponding source. Copies distributed under
