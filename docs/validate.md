...
**Every finding names its own severity, and every run ends with its errors.** A line opens with `error:` or `warning:`
the way `rustc` and `clippy` write one, before the `path:line`, the rule code, and the message:
...
The marker leads the line because paths vary in width, so anywhere later it stops being a column that can be scanned.
That matters because a whole-tree run is routinely a couple of hundred advisories with a single error somewhere inside
them.

The listing itself stays in tree order — per pass, per file, per line — so a file's findings remain adjacent. A run that
found any error therefore closes with an errors-only recapitulation, after every pass has printed:
...
The recapitulation spans all six passes, which is why it is a separate block rather than a reordering: the four
standalone passes print *after* the markdown lint's summary line, so no ordering within one pass could gather them.
Reading it is the supported way to answer "which line do I fix" — the summary count and the exit code say only *how
many*.
