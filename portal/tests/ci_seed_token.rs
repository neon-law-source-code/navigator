...
async fn minting_a_ci_token_scopes_it_to_seed_and_the_named_project() {
    ...
    let exp = minted["exp"].as_i64().expect("exp");
    ...
    .unwrap();
    assert!(
        (now + 9 * 60..now + 11 * 60).contains(&exp),
        "CI seed tokens last ten minutes, got exp {exp} at now {now}"
    );
...
#[tokio::test]
async fn a_replayed_oidc_token_is_refused() {
    let (app, github_token) = fixture_app().await;
    assert_eq!(mint(&app, &github_token).await.status(), StatusCode::OK);
    let replay = mint(&app, &github_token).await;
    assert_eq!(replay.status(), StatusCode::UNAUTHORIZED);
    let body = axum::body::to_bytes(replay.into_body(), usize::MAX)
        .await
        .unwrap();
    let document: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(document["error"], "unauthorized");
}

#[tokio::test]
async fn an_unbound_repository_is_refused_without_naming_a_project() {
    let surreal = mem_surreal().await;
    let mut state = portal::test_support::app_state(surreal).await;
    state.canonical_host = portal::CanonicalHost::new(Some("staging.neonlaw.com".into()));
    state.github_oidc = GitHubOidc::fixed(GitHubActionsClaims {
        repository: "neon-law-staging/missing".into(),
        repository_owner: "neon-law-staging".into(),
        jti: "jti-missing".into(),
        exp: 4_000_000_000,
        ..GitHubActionsClaims::default()
    });
    ...
    assert!(text.contains("not bound to a live project"), "{text}");
    assert!(!text.contains("missing"), "{text}");
}

#[tokio::test]
async fn two_projects_sharing_one_repository_url_are_refused() {
    let surreal = mem_surreal().await;
    let first = store::projects::create(
        &surreal,
        &store::projects::NewProject {
            code: "acme".into(),
            name: "Acme".into(),
    ...
    store::projects::set_repository_url(
        &surreal,
        second.id,
        Some("https://github.com/neon-law-staging/acme.git"),
    )
    .await
    .unwrap();
    let mut state = portal::test_support::app_state(surreal).await;
