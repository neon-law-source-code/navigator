...
narration units. A notation the firm has given a client narrates on that stage at
`/app/projects/{code}/{notation_id}/outline`. `N123` holds every kind that must carry an outline to its depth-1 scheme.
