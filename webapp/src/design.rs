    ...
    rsx! {
        section {
            ...
            p {
                ...
                "JavaScript. The real route redirects an uploaded firm-tier public avatar URL, streams a private "
                "client avatar, or serves an initials fallback from the authenticated session."
            }
...
fn demo_memberships() -> Vec<FooterMembership> {
    vec![FooterMembership {
        ...
        standing: "Mission-Aligned Partner".to_string(),
...
mod tests {
    ...
    fn yaml_snippet_is_a_real_frontmatter() {
        let crate_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let path = crate_root.join("../templates/notations/neon_law/onboarding.md");
        let source = std::fs::read_to_string(&path)
            .unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
        assert!(
