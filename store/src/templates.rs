...
mod tests {
    ...
    async fn save_version_persists_and_reads_back_the_kind() {
        ...
        save_version(&db, None, "onboarding__engagement_letter", v)
            .await
            .unwrap();
        let current = resolve(&db, None, "onboarding__engagement_letter")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn a_padded_kind_is_stored_trimmed_so_the_retainer_gate_matches() {
        ...
        assert_eq!(
            resolve(&db, None, "onboarding__engagement_letter")
                .await
                .unwrap()
                .unwrap()
                .kind
