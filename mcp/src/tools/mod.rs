...
//!
//! Tool names are bare — `create_person`, not `navigator_create_person`.
//! A client that surfaces several MCP servers at once (Gemini
//! Enterprise, `LibreChat`) groups them by the server rather than by the
//! tool name, and this server announces itself as `navigator-mcp` in
//! `serverInfo`. The A2A skill id is the same string, so nothing has to
//! translate between the two surfaces.
...
pub mod delete_closed_repository;
...
pub fn list_tools() -> Vec<Value> {
    vec![
        ...
        delete_closed_repository::descriptor(),
        ...
        spawn_legal_council::descriptor(),
        send_welcome_email::descriptor(),
    ]
}
...
/// and `list_tools` and the A2A agent card still describe every
...
/// to a matter repo. The MCP tool name and the A2A skill id are the same
/// string (`create_person`), so there is nothing to normalize. Tools not
/// listed in [`READ_ONLY_TOOLS`] default to side-effecting, so the safe
/// answer is the default for anything new or unrecognized.
#[must_use]
...
pub fn requires_confirmation(tool_name: &str) -> bool {
    is_side_effecting(tool_name) && !CONFIRMATION_EXEMPT_TOOLS.contains(&tool_name)
}

/// Whether `tool_name` names a real tool in the catalog. Callers that
/// gate side-effecting tools use this so an *unknown* skill still falls
/// through to the `Unknown` error rather than being reported as an
/// authorization failure.
#[must_use]
pub fn is_known_tool(tool_name: &str) -> bool {
    list_tools()
        .iter()
        .any(|d| d.get("name").and_then(Value::as_str) == Some(tool_name))
}
...
pub async fn call_tool(
    ...
    match name {
        "create_person" => create_person::call(surreal, arguments).await,
        "show_person" => show_person::call(surreal, &scope, arguments).await,
        "list_jurisdictions" => list_jurisdictions::call(surreal, arguments).await,
        "list_entities" => list_entities::call(surreal, arguments).await,
        "create_notation" => {
        ...
        "close_project" => close_project::call(surreal, arguments).await,
        "delete_closed_repository" => {
            delete_closed_repository::call(surreal, state.forge.as_ref(), arguments).await
        }
        "list_deadlines" => list_deadlines::call(surreal, &scope, arguments).await,
        "list_projects" => list_projects::call(surreal, &scope, arguments).await,
        "project_status" => project_status::call(surreal, &scope, arguments).await,
        "link_person_project" => link_person_project::call(surreal, arguments).await,
        "bulk_import" => bulk_import::call(surreal, principal, arguments).await,
        "list_tools" => list_tools::call(arguments).await,
        "spawn_legal_council" => spawn_legal_council::call(arguments).await,
        "send_welcome_email" => send_welcome_email::call(state, arguments).await,
...
#[cfg(test)]
mod tests {
    use super::{call_tool, list_tools, resolve_caller, Principal, ReadScope, Role, ToolError};
    use crate::server::McpState;
    use serde_json::json;
    use std::sync::Arc;
    ...
    /// [`list_tools`] is named bare and in `snake_case`. This runs over
    /// *whatever* `list_tools` returns, so a future tool that
    /// reintroduces a vendor prefix fails here without anyone having to
    /// remember to update the explicit set below.
    #[test]
    fn every_tool_name_is_bare_snake_case() {
        ...
        for tool in &tools {
            ...
            assert!(!name.is_empty(), "tool descriptor has an empty `name`");
            assert!(
                name.chars()
                    .all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || c == '_'),
                "tool name `{name}` must be lowercase `snake_case`",
            );
            for prefix in ["aida_", "navigator_", "mcp_", "neon_"] {
                assert!(
                    !name.starts_with(prefix),
                    "tool name `{name}` carries the `{prefix}` vendor prefix; \
                     the server identifies itself through `serverInfo`, not through \
                     every tool name",
                );
            }
        }
    ...
    /// [`every_tool_name_is_bare_snake_case`] — that one enforces the
    /// convention, this one pins the *contents* so a tool can't be
    /// silently removed.
    #[test]
    fn list_tools_advertises_the_expected_registry() {
        ...
        assert!(names.contains(&"create_notation"));
        assert!(names.contains(&"answer_notation"));
        assert!(names.contains(&"validate_notation"));
        assert!(names.contains(&"create_project"));
        assert!(names.contains(&"close_project"));
        assert!(names.contains(&"delete_closed_repository"));
        assert!(names.contains(&"list_deadlines"));
        assert!(names.contains(&"list_projects"));
        assert!(names.contains(&"project_status"));
        assert!(names.contains(&"link_person_project"));
        assert!(names.contains(&"bulk_import"));
        assert!(names.contains(&"list_tools"));
        assert!(names.contains(&"spawn_legal_council"));
        assert!(names.contains(&"send_welcome_email"));
    }
    ...
    fn writers_are_side_effecting_and_default_is_safe() {
        ...
        assert!(super::is_side_effecting("some_future_writer"));
    ...
    fn is_advertised_separates_gated_from_unknown() {
        ...
        assert!(!super::is_advertised("create_notation"));
        assert!(super::is_known_tool("create_notation"));

        assert!(!super::is_advertised("not_a_tool"));
        assert!(!super::is_known_tool("not_a_tool"));
    }

    #[test]
    fn withheld_message_names_the_tool_and_where_to_perform_it() {
        let text = super::withheld_message("send_welcome_email");
        assert!(text.contains("send_welcome_email"), "got `{text}`");
    ...
    fn requires_confirmation_gates_client_facing_and_notation_writes() {
        ...
        for name in ["send_welcome_email", "create_notation", "answer_notation"] {
            assert!(
                super::requires_confirmation(name),
    ...
    fn requires_confirmation_is_fail_closed_for_an_unclassified_tool() {
        ...
        assert!(super::requires_confirmation("some_future_writer"));
    ...
    async fn require_tool_authz_blocks_non_lawyer_yet_allows_anonymous_and_read_only() {
        ...
        assert!(authz(&s.surreal, None, "create_project").await.is_ok());
        // A read-only tool is never gated on tier.
        assert!(authz(&s.surreal, Some(&client), "show_person")
            .await
            .is_ok());
        // A side-effecting tool by an authenticated client-tier caller is
        // refused — the core of the fix.
        assert!(matches!(
            authz(&s.surreal, Some(&client), "create_project").await,
    ...
    async fn require_tool_authz_matches_lawyer_email_case_insensitively() {
        ...
        assert!(
            authz(&s.surreal, Some(&caller), "create_project")
                .await
                .is_ok(),
    ...
    #[tokio::test]
    async fn call_tool_dispatches_validate_notation() {
        let s = state().await;
    ...
    async fn a_read_tool_is_not_gated_by_the_write_tier_check() {
        ...
        let read = call_tool(&state, Some(&principal), "list_projects", &json!({}))
            .await
            .unwrap();
