...
command, the `bulk_import` MCP tool, or the web upload route. Use this when someone hands you a contact list (names,
emails, titles, phone numbers, org names) and wants those people and their organizations created as `persons` and
`entities` with the links between them. Trigger on "bulk import these contacts", "load this list of people and orgs", or
"create these as clients", or when Navigator MCP is asked to add several people at once. The contract and rules live in
`docs/bulk-contact-import.md`; this skill is the LLM-facing recipe for producing the JSON and choosing the surface. Skip
for a single person (use `create_person`) or for opening a matter/Project (the per-engagement onboarding flow, not this
importer). ---
...
- **Navigator MCP (chat / MCP):** call `bulk_import` with the payload as the arguments. Lawyer/admin only; a client-tier
  or anonymous caller is refused. It returns a per-row created/updated/unchanged/failed report.
